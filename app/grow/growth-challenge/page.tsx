...
export default function GrowthChallenge() {
  ...
  const handleSelection = (challenge: string) => {
    saveFunnelData('growthChallenge', challenge);
    router.push('/grow/business-type');
  };
  ...
  const challenges = [
    { icon: "💸", text: "Creators never respond to my DMs" },
    { icon: "📱", text: "Don't know which creators will convert" },
    { icon: "🔥", text: "Wasting hours on DMs with no results" },
    { icon: "⏳", text: "Too expensive to test properly" },
  ];
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-6 md:px-4 md:py-8">
        <div className={`w-full max-w-3xl transition-all duration-1000 ${
          ...
          <h1 className={`text-lg md:text-2xl text-center mb-6 md:mb-8 font-bold leading-tight max-w-3xl mx-auto transition-colors ${
            ...
            What's your biggest problem with influencer marketing?
          </h1>
          ...
          <div className="grid md:grid-cols-2 gap-2 md:gap-3 max-w-2xl mx-auto">
            {challenges.map((challenge, index) => (
              <button
                key={index}
                onClick={() => handleSelection(challenge.text)}
              ...
              >
                <span className="text-xl md:text-2xl flex-shrink-0">{challenge.icon}</span>
