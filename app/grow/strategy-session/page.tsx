...
export default function StrategySession() {
  ...
  const handleYes = () => {
    saveFunnelData('wantsSession', 'Yes');
    router.push('/grow/book-call');
  };

  const handleNo = () => {
    saveFunnelData('wantsSession', 'No');
    router.push('/grow/not-ready');
  };
    ...
    <main className={`relative min-h-screen flex flex-col overflow-x-hidden ${
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-6 md:px-4 md:py-8">
        <div className={`w-full max-w-3xl transition-all duration-1000 ${
          ...
          {/* Main question */}
          <h1 className={`text-lg md:text-2xl text-center mb-4 font-bold leading-tight max-w-3xl mx-auto transition-colors ${
            isDark ? 'text-white' : 'text-black'
          }`}>
            Want us to find the right creators for you?
          </h1>

          {/* Description */}
          <p className={`text-center text-xs md:text-sm mb-6 md:mb-8 max-w-2xl mx-auto transition-colors ${
            isDark ? 'text-zinc-400' : 'text-zinc-600'
          }`}>
            We'll audit your niche, find creators that actually convert, and show you exactly how to run profitable influencer campaigns – without wasting time on DMs or fake followers.
          </p>
          ...
          <div className="grid md:grid-cols-2 gap-3 max-w-2xl mx-auto">
            <button
              onClick={handleYes}
              className={`group relative p-4 md:p-5 rounded-xl transition-all duration-500 ease-out hover:scale-[1.02] active:opacity-95 text-left w-full ${
                isDark
            ...
            >
              <div className="flex flex-col items-center text-center gap-3">
                <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-all duration-500 bg-green-500/20`}>
